...
async def upgrade(ctx: Context, item_index: int, *args):
    ...
    start_item_data = database.item_data["items"][start_item_name]

    # check the item they want to upgrade too exists
    try:
        result_item_data = database.item_data["items"][result_item_name]
    except KeyError:
        # try and find closest match
        closest_match = get_closest_match(
            result_item_name, database.item_data["items"].keys()
        )
