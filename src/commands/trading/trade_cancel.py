...
async def cancel(ctx:Context, recipient:discord.Member):
    ...
    if recipient is None:
        delete_result = database.trade_requests.delete_one({"_id": ctx.author.id, "send_timestamp": 0})
        ...
        delete_result = database.trade_requests.delete_one({"_id": ctx.author.id, "recipient_id": recipient.id})
        successful = delete_result.deleted_count >= 1

        if successful:
            await msg_embed(ctx, get_locale(lang, "trade_cancel.cancelled_trade_to_user", recipient.name ))
        else:
